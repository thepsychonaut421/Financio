...
interface BankStatementUploadFormProps {
...
export function BankStatementUploadForm({ onFilesSelected, onProcess, isProcessing, selectedFileCount }: BankStatementUploadFormProps) {
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      if (newFiles.length !== event.target.files.length) {
        // Optionally inform user if non-PDFs were filtered out
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Select one or more PDF files of bank statements to extract transactions.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <Button
          ...
          size="lg"
        >
          {isProcessing ? 'Processing...' : `Extract from ${selectedFileCount} File${selectedFileCount === 1 ? '' : 's'}`}
        </Button>
      </CardContent>
    </Card>
  );
