...
interface ProcessingResult extends SuggestPdfFilenameOutput {
  id: string; // Unique ID for each file processed
  originalName: string;
  dataUri: string;
}
...
export default function PdfOrganizerPage() {
  ...
  const [editableFilenames, setEditableFilenames] = useState<Record<string, string>>({}); // Keyed by ProcessingResult.id
  ...
  const handleProcessFiles = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      try {
        ...
        // Optionally, add per-file error to results
        results.push({ 
            id: fileId, 
            originalName: file.name, 
  ...
  const handleDownloadAll = async () => {
    ...
    if (processingResults.length === 1) {
      ...
      for (const result of processingResults) {
        if (result.dataUri && result.dataUri.startsWith('data:')) { // Ensure it's a valid data URI and not an error placeholder
          try {
