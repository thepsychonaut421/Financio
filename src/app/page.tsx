...
import type { ExtractedItem as AIOutputItem } from '@/ai/flows/extract-invoice-data'; // This is ExtractedItem from AI
import type { ExtractedItem, ProcessingStatus } from '@/types/invoice'; // This is the final type for display
...
export default function PdfExtractorPage() {
  ...
  const handleFilesSelected = useCallback((files: File[]) => {
    ...
    setExtractedItems([]); // Clear previous results when new files are selected
    setStatus('idle');
  ...
  const handleProcessFiles = async () => {
    ...
    try {
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        setProgress(Math.round(((i + 1) / selectedFiles.length) * 90)); // 90% for extraction
      }
      ...
      if (allInvoiceDetails.length > 0) {
        ...
        setExtractedItems([]); // Set to empty if no details were extracted
      }
